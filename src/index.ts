...
import type { ModelProvider, Plan } from "./providers";
import * as utils from "./utils";
import { executePrompt } from "./providers/index";
...
${directoryTree}
  ...
  async executeActions(actionEvents: ActionEvent[]) {
    ...
    console.log("executeActions - actionEvents:", actionEvents);
  ...
  async prompt(userInput: string): Promise<PromptReturnValue> {
    ...
    const actionReturnValues = await this.executeActions(actionEvents.actions);
    return {
      userInput,
      plan: actionEvents.actions,
      actionEvents: actionEvents.actions,
      actionReturnValues,
      instance: this,
    };
  }
